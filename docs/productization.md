...
The protocol-v2 corrective run reached real C1, W1, and C2 activations with all three durable ActivationAcks, mailbox claims/completions, and the Worker result payload. C2 returned `FULL_LOOP_COMPLETE` in its host turn and verified the Worker result, but did not write the required durable terminal result. The driver preserved this as a semantic terminal-output failure and did not synthesize the result or start C3; therefore the full endpoint roundtrip PASS remains unclaimed.
